...
func main(){logger:=slog.New(slog.NewJSONHandler(os.Stdout,nil));cfg,err:=config.Load();if err!=nil{logger.Error("configuration error","error",err);os.Exit(1)};store:=supabase.New(cfg.SupabaseURL,cfg.SupabaseServiceRoleKey);fallback:=generic.New();propixClient:=propix.New(cfg.ProPixClientID,cfg.ProPixClientSecret,cfg.ProductName,gateway.DefaultCustomerEmail);registry:=gateway.NewRegistry(fallback,cashinpay.New(cfg.CashinPaySecretKey,cfg.CashinPayWebhookSecret,cfg.ProductName,gateway.DefaultCustomerEmail),propixClient,m2pay.New(cfg.M2PayAPIKey,cfg.ProductName,gateway.DefaultCustomerEmail),nowbanks.New(cfg.NowBanksClientID,cfg.NowBanksClientSecret,cfg.NowBanksWebhookSecret),staticpix.New(cfg.PIXKey,cfg.PIXReceiver,cfg.PIXCity));paymentRouter:=payment.NewWithExpiration(store,registry,cfg.ProductName,cfg.PIXExpiration);generator:=payment.NewGenerator(store,paymentRouter);confirmer:=payment.NewConfirmer(store,registry);status:=payment.NewStatusService(store,confirmer);webhooks:=payment.NewWebhookService(confirmer);adminAuth:=authsvc.New(cfg.SupabaseURL,cfg.SupabasePublishableKey,store);adminInvoices:=admin.NewInvoiceService(store);adminImporter:=admin.NewImporter(store);adminMetrics:=admin.NewMetricsService(store);adminActivity:=admin.NewActivityService(store);adminGateways:=admin.NewGatewayAdminService(store,os.Getenv);legacyAPI:=legacy.New(store,paymentRouter,propixClient,cfg.SiteURL);handler:=httpapi.New(httpapi.Dependencies{Invoices:invoice.New(store),PIX:generator,Status:status,Webhooks:webhooks,Access:store,Auth:adminAuth,AdminInvoices:adminInvoices,AdminImporter:adminImporter,AdminMetrics:adminMetrics,AdminActivity:adminActivity,AdminGateways:adminGateways,Legacy:legacyAPI,SiteURL:cfg.SiteURL},logger);server:=&http.Server{Addr:cfg.Addr,Handler:handler,ReadHeaderTimeout:5*time.Second,ReadTimeout:15*time.Second,WriteTimeout:35*time.Second,IdleTimeout:90*time.Second};logger.Info("server started","addr",cfg.Addr);if err:=server.ListenAndServe();err!=nil&&err!=http.ErrServerClosed{logger.Error("server stopped","error",err);os.Exit(1)}}
