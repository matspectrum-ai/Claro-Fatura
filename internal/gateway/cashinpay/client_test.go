...
func TestCreatePIXMatchesCurrentCashinPayPayloadShape(t *testing.T) {
	...
	if created.TransactionID != "tx-123" || created.CopyPaste != "000201PIXCODE" { t.Fatalf("created=%+v", created) }
	if got["amount"].(float64) != 123.45 { t.Fatalf("amount=%v", got["amount"]) }
	if got["transaction_id"] != "req-1" || got["description"] != "Ebook Viver de Vendas" || got["postbackUrl"] != "https://site/webhook" { t.Fatalf("payload=%v", got) }
	customer := got["customer"].(map[string]any)
	if customer["name"] != "Maria" || customer["email"] != "cliente@ebookviver.app" || customer["phone"] != "93991234567" { t.Fatalf("customer=%v", customer) }
}
...
func TestPaidStatusCompatibility(t *testing.T) {
	...
	for _, value := range []string{"paid", "APPROVED", "completed", "confirmed", "pago", "aprovado"} {
		v := value
		if !client.Paid(&v) { t.Fatalf("expected %q paid", value) }
	}
	v := "pending"
	if client.Paid(&v) { t.Fatal("pending must not be paid") }
}
