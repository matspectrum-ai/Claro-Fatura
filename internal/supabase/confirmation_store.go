...
func (c *Client) GatewayBySlug(ctx context.Context, slug string) (gateway.Record, bool, error) { var rows []gateway.Record; q:=url.Values{"select":{"id,slug,rotulo,adapter,ativo,prioridade,api_url,ambiente,limite_diario,webhook_url,secret_names,observacoes"},"slug":{"eq."+slug},"limit":{"1"}}; if err:=c.Select(ctx,"gateways_config",q,&rows);err!=nil{return gateway.Record{},false,err};if len(rows)==0{return gateway.Record{},false,nil};return rows[0],true,nil }
...
func (c *Client) CancelOtherPendingTransactions(ctx context.Context,invoiceID,exceptID string,at time.Time)error{return c.Update(ctx,"transacoes_pix",url.Values{"fatura_id":{"eq."+invoiceID},"status":{"eq.pendente"},"id":{"neq."+exceptID}},map[string]any{"status":"cancelada","substituida_em":at.Format(time.RFC3339Nano)})}
...
func (c *Client) UpdateTransactionStatus(ctx context.Context,id,status string)error{return c.Update(ctx,"transacoes_pix",url.Values{"id":{"eq."+id}},map[string]any{"status":status})}
func (c *Client) InsertWebhookLog(ctx context.Context,log payment.WebhookLog)error{return c.Insert(ctx,"webhooks_log",map[string]any{"gateway_slug":log.GatewaySlug,"evento":log.Event,"transacao_gateway_id":log.GatewayTransaction,"assinatura_valida":log.SignatureValid,"resumo":log.Summary})}
