...
func (c *Client) RequestTransaction(ctx context.Context, reservation payment.RequestReservation) (*payment.Transaction, error) {
	if reservation.Status != "concluida" || reservation.TransactionID == nil {
		return nil, nil
	}
	q := url.Values{"select": {"id,fatura_id,cliente_id,gateway_slug,gateway_id,transacao_gateway_id,valor_centavos,copia_cola,qrcode,status,expira_em"}, "id": {"eq." + *reservation.TransactionID}, "limit": {"1"}}
...
func (c *Client) ActiveGateways(ctx context.Context) ([]gateway.Record, error) {
	...
	q := url.Values{
		...
		"ativo":  {"eq.true"}, "order": {"prioridade.asc"},
	}
...
func (c *Client) InsertTransaction(ctx context.Context, in payment.TransactionCreate) (payment.Transaction, error) {
	body := map[string]any{
		"fatura_id": in.InvoiceID, "cliente_id": in.CustomerID, "gateway_slug": in.GatewaySlug,
		"gateway_id": in.GatewayID, "transacao_gateway_id": in.GatewayTransactionID,
		"valor_centavos": in.AmountCents, "copia_cola": in.CopyPaste, "qrcode": in.QRCode,
		"status": in.Status, "idempotency_key": in.IdempotencyKey, "expira_em": in.ExpiresAt,
	}
...
func (c *Client) Log(ctx context.Context, entry payment.LogEntry) error {
	return c.Insert(ctx, "pagamentos_log", map[string]any{
		"gateway_slug": entry.GatewaySlug, "fatura_id": entry.InvoiceID, "nivel": entry.Level,
		"http_status": entry.HTTPStatus, "mensagem": entry.Message,
	})
}
...
func transaction(row transactionRow) payment.Transaction {
	return payment.Transaction{
		ID: row.ID, InvoiceID: row.InvoiceID, CustomerID: row.CustomerID, GatewaySlug: row.GatewaySlug,
		GatewayID: row.GatewayID, GatewayTransactionID: row.GatewayTransactionID, AmountCents: row.AmountCents,
		CopyPaste: row.CopyPaste, QRCode: row.QRCode, Status: row.Status, ExpiresAt: row.ExpiresAt,
	}
}
