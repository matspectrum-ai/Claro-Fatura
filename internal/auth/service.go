...
func (s *Service) SignUp(ctx context.Context, name, email, password, redirectTo string) (Session, bool, error) {
	...
	err := s.doJSON(ctx, http.MethodPost, endpoint, "", map[string]any{
		"email": strings.TrimSpace(email), "password": password, "data": map[string]any{"nome": strings.TrimSpace(name)},
	}, &response)
	if err != nil {
	...
	if response.User.ID == "" && response.ID != "" {
		response.User = User{ID: response.ID, Email: response.Email}
	}
	admin, err := s.store.IsAdmin(ctx, response.User.ID)
	if err != nil {
		return Session{}, false, err
	}
	if !admin {
		return Session{}, false, ErrNotAdmin
	}
	return response.Session, false, nil
}
